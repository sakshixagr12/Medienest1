...
function PricingPageContent() {
    ...
    (async () => {
      try {
        ...
        if (userClinic.status === "active" && !orderId) {
          const dest = userClinic.clinic_type === "store" ? "/demo1/store" : "/demo1/portal";
          router.replace(dest);
          return;
        }
  ...
  const verifyPaymentOnServer = async (orderId: string, currentClinic: any) => {
    ...
    try {
      ...
      if (data.success && data.order_status === "PAID") {
        ...
        const dest = currentClinic.clinic_type === "store" ? "/demo1/store" : "/demo1/portal";
        setTimeout(() => {
  ...
  const handleStartTrial = async (planName: string) => {
    ...
    try {
      ...
      setVerificationSuccess(true);
      const dest = clinic.clinic_type === "store" ? "/demo1/store" : "/demo1/portal";
      setTimeout(() => {
        window.location.href = dest;
      }, 1500);
    } catch (err: any) {
