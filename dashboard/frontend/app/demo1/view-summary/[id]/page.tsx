...
import styles from "@/app/demo1/portal/discharge-summary/view/view.module.css";

export default function PublicDischargeSummary({
  params,
}: {
    ...
    async function fetchRecord() {
      try {
        ...
        const parsedSummary = {
          ...
          complaints:
            typeof record.complaints === "string"
              ? JSON.parse(record.complaints)
              : record.complaints || [],
          findings:
            typeof record.findings === "string"
              ? JSON.parse(record.findings)
              : record.findings || [],
          treatment:
            typeof record.treatment === "string"
              ? JSON.parse(record.treatment)
              : record.treatment || [],
          medicines:
            typeof record.medicines === "string"
              ? JSON.parse(record.medicines)
              : record.medicines || [],
          advice:
            typeof record.advice === "string"
              ? JSON.parse(record.advice)
              : record.advice || [],
        };
  ...
  if (error || !summary) {
    return (
      ...
      >
        ...
        <p>
          We could not find this Discharge Summary. It may have been deleted or
  ...
  return (
    <div className={styles.page}>
      ...
      <header className={styles.header}>
        <div
          style={{
            color: "var(--sanctuary-ink-m)",
            fontSize: 13,
            fontWeight: 600,
          }}
        >
          MedieNest Secure Encrypted Link
        </div>
              ...
              <tr>
                <td>
                  <div className={styles.previewHeader}>
                    <h2>{clinic?.name || "MedieNest Partner Clinic"}</h2>
                    <p>{clinic?.address}</p>
                  ...
                  <div className={styles.previewInfoGrid}>
                    ...
                    <div>
                      <b>DOA:</b>{" "}
                      {summary.doa
                        ? new Date(summary.doa).toLocaleDateString("en-IN")
                        : "--"}
                    </div>
                    <div>
                      <b>DOD:</b>{" "}
                      {summary.dod
                        ? new Date(summary.dod).toLocaleDateString("en-IN")
                        : "--"}
                    </div>
                  </div>
                </td>
                ...
                <td>
                  ...
                  {summary.complaints.length > 0 && (
                    <div className={styles.previewSection}>
                      ...
                      <ul className={styles.previewList}>
                        {summary.complaints.map((item: string, i: number) => (
                          <li key={i}>{item}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {summary.findings.length > 0 && (
                    <div className={styles.previewSection}>
                      ...
                      <ul className={styles.previewList}>
                        {summary.findings.map((item: string, i: number) => (
                          <li key={i}>{item}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {summary.treatment.length > 0 && (
                    <div className={styles.previewSection}>
                      ...
                      <ul className={styles.previewList}>
                        {summary.treatment.map((item: string, i: number) => (
                          <li key={i}>{item}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {summary.advice.length > 0 && (
                    ...
                    >
                      ...
                      <ul className={styles.previewList}>
                        {summary.advice.map((item: string, i: number) => (
                          <li key={i}>{item}</li>
                        ))}
                      </ul>
                    </div>
