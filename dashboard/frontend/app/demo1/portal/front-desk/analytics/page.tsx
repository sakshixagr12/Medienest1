...
export default function AnalyticsDashboardPage() {
  ...
  const computedRanges = useMemo(() => {
    ...
    return {
      ...
      compare: validCompare
        ? { start: cStart.toISOString(), end: cEnd.toISOString() }
        : null,
    };
  }, [
    baseFilter,
  ...
  async function fetchAnalytics() {
    if (!clinic) return;
    if (baseFilter === "Custom..." && (!baseCustomStart || !baseCustomEnd))
      return;
    ...
    try {
      ...
      const response = await authenticatedFetch(
        `${API_BASE_URL}/api/analytics/dashboard?${queryParams.toString()}`,
      );
      ...
      setError("Network error. Ensure the MedieNest backend is running.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  const handleDiagnosisClick = async (diagnosis: string) => {
    ...
    try {
      ...
      const res = await authenticatedFetch(
        `${API_BASE_URL}/api/analytics/patients?${queryParams.toString()}`,
      );
  ...
  const renderTrend = (trend: number | null) => {
    ...
    return (
      <span className={`${styles.trendBadge} ${styles.trendNeutral}`}>0%</span>
    );
  };
  ...
  const renderLineChart = () => {
    ...
    if (tData.length === 0) {
      tData = [
        {
          date: "Start",
          ...top3.reduce((acc, d) => ({ ...acc, [d.diagnosis!]: 0 }), {}),
        },
        {
          date: "End",
          ...top3.reduce(
            (acc, d) => ({ ...acc, [d.diagnosis!]: Number(d.count) }),
            {},
          ),
        },
      ];
    ...
    // 3. Scale logic
    const allVals = tData.flatMap((pt) =>
      top3.map((d) => Number(pt[d.diagnosis!] || 0)),
    );
  ...
  const renderContent = () => {
    ...
    if (!data)
      return (
        <div className={styles.emptyState}>No diagnostic data available.</div>
      );

    const maxDiagnosis = Math.max(
      ...(data.diagnoses.map((d) => Number(d.count)) || [1]),
    );
    const maxMedicine = Math.max(
      ...(data.medicines.map((m) => Number(m.count)) || [1]),
    );
    const maxRevenue = Math.max(
      ...(data.revenueTimeline?.map((r) => Number(r.amount)) || [1]),
    );
    const totalDemographics = data.demographics.reduce(
      ...
      <div className={styles.mainLayout}>
          ...
          <div className={styles.glassCard} style={{ padding: "2rem" }}>
            ...
            <div className={styles.metricGrid}>
              ...
              <div className={`${styles.metricBox} ${styles.boxLavender}`}>
                ...
                <div className={styles.metricSubtext}>
                  {isComparisonMode && baseFilter !== "All Time" ? (
                    <span>
                      vs prev: {data.summary?.prevTotalPatients}{" "}
                      {renderTrend(data.summary?.patientsTrend)}
                    </span>
                  ) : (
                    "Patient Volume"
                  )}
              ...
              <div className={`${styles.metricBox} ${styles.boxTeal}`}>
                ...
                <div className={styles.metricValue}>
                  {data.summary?.avgConsultTime || "--"}
                </div>
                <div className={styles.metricSubtext}>
                  {isComparisonMode && baseFilter !== "All Time" ? (
                    <span>
                      vs prev: {data.summary?.prevConsultTime}{" "}
                      {renderTrend(data.summary?.consultTrend)}
                    </span>
                  ) : (
                    "Process Efficiency"
                  )}
              ...
              >
                ...
                <div className={styles.metricValue}>
                  ₹{(data.summary?.revenue || 0).toLocaleString()}
                </div>
                <div className={styles.metricSubtext}>
                  {isComparisonMode && baseFilter !== "All Time" ? (
                    <span>
                      vs prev: ₹{data.summary?.prevRevenue?.toLocaleString()}{" "}
                ...
                <div className={styles.barList}>
                  {(showAllDiagnoses
                    ...
                    <div
                      ...
                      onClick={() => handleDiagnosisClick(item.diagnosis!)}
                    >
                      ...
                      <div className={styles.barTrack}>
                        <div
                          ...
                          style={{
                            width: `${(Number(item.count) / maxDiagnosis) * 100}%`,
                            background: "#818cf8",
                          }}
                        />
                      </div>
                  ...
                  {data.diagnoses.length > 5 && (
                    ...
                    >
                      {showAllDiagnoses
                        ? "Show Less ⬆️"
                        : `Show All Diagnoses (${data.diagnoses.length}) ⬇️`}
                    </button>
                  )}
                </div>
              ) : (
                  ...
                  <div className={styles.barList}>
                      ...
                      >
                        <div className={styles.barInfo}>
                          ...
                          >
                            ...
                            {item.isOverused && (
                              <span className={styles.overusedChip}>️ High</span>
                            )}
                          </div>
                          <span className={styles.countLg}>{item.count}</span>
                        </div>
                        <div className={styles.barTrack}>
                          <div
                            className={styles.barFill}
                            style={{
                              width: `${(Number(item.count) / maxMedicine) * 100}%`,
                              background: "#34d399",
                            }}
                          />
                        </div>
                    ...
                    {data.medicines.length > 5 && (
                      ...
                      >
                        {showAllMedicines
                          ? "Show Less ⬆️"
                          : `Show All Medicines (${data.medicines.length}) ⬇️`}
                      </button>
                    )}
                  </div>
                ) : (
        ...
        <div className={styles.rightColumn}>
              ...
              <>
                ...
                >
                  <div className={styles.pieTotal}>
                    <div className={styles.pieTotalNum}>
                      {totalDemographics}
                    </div>
          ...
          <div className={styles.glassCard}>
            ...
            {data.advancedDemographics &&
            data.advancedDemographics.length > 0 ? (
                  ...
                  <div key={ag.ageGroup} className={styles.recentItem}>
                    <div style={{ display: "flex", alignItems: "center" }}>
                      <div className={styles.recentAvatar}>
                        {ag.ageGroup.split("-")[0] || ag.ageGroup}
                      </div>
    ...
    <DashboardLayout>
        ...
        <div className={styles.container}>
          ...
          >
            <div
              ...
              style={{ marginBottom: 0, flex: "1 1 400px" }}
            >
              <h1>Clinical Intelligence</h1>
              <p>
                Advanced diagnostic and financial oversight for {clinic?.name}
              </p>
            </div>
                  ...
                  <label className={styles.compareToggle}>
                    <input
                      ...
                      style={{
                        width: "16px",
                        height: "16px",
                        accentColor: "var(--sanctuary-primary)",
                      }}
                    />
                    Compare Mode
                  </label>
              ...
              {baseFilter === "Custom..." && (
                ...
                >
                  ...
                  <input
                    ...
                    value={baseCustomStart}
                    onChange={(e) => setBaseCustomStart(e.target.value)}
                  />
                  <span style={{ fontSize: "0.8rem", fontWeight: 700 }}>
                    to
                  </span>
                  <input
                    ...
                    style={{ padding: "4px 8px" }}
                    value={baseCustomEnd}
                    onChange={(e) => setBaseCustomEnd(e.target.value)}
                  />
                </div>
              )}
                ...
                modalPatients.map((p) => (
                  <div key={p.id} className={styles.modalPatientItem}>
                    <div>
                      ...
                      <div className={styles.patSub}>
                        {p.age}Y • {p.gender} • {p.phone_number || "No contact"}
                      </div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      ...
                      <Link
                        href={`/demo1/portal/doctor-dashboard/patients/${p.id}`}
                        style={{
                          fontSize: "0.8rem",
                          fontWeight: 800,
                          color: "var(--sanctuary-primary)",
      ...
      {revenueDrawerOpen && data && (
        <div
          ...
          onClick={() => setRevenueDrawerOpen(false)}
        >
          <div
          ...
          >
            ...
            <div className={styles.drawerBody}>
              <div className={styles.revenueDetailCard}>
                ...
                >
                  Total Revenue ({baseFilter})
                </div>
                <div className={styles.revenuePrimaryValue}>
                    ...
                    {data.revenueTimeline && data.revenueTimeline.length > 0
                      ? (() => {
                          const maxRev = Math.max(
                            1,
                            ...data.revenueTimeline.map((r) => r.amount),
                          );
                          const points = data.revenueTimeline
                            .map((pt, idx) => {
                              const x =
                                (idx /
                                  (data.revenueTimeline!.length - 1 || 1)) *
                                300;
                ...
                <div className={styles.barChart}>
                  {data.revenueTimeline && data.revenueTimeline.length > 0 ? (
                    data.revenueTimeline.slice(-7).map((pt, idx) => {
                      ...
                      const day = new Date(pt.date).toLocaleDateString(
                        "en-US",
                        { weekday: "short" },
                      );
                      return (
              ...
              <div style={{ marginTop: "2rem" }}>
                ...
                >
                  ...
                  >
                    ...
                    <span
                      style={{ fontWeight: 800, color: "var(--sanctuary-ink)" }}
                    >
                      ₹
                      {(
                        data.summary.revenue / (data.summary.totalPatients || 1)
                      ).toFixed(0)}
                    </span>
                  </div>
                  <div
