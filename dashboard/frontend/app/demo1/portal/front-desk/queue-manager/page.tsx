...
export default function QueueManagerPage() {
  ...
  const [checkInPriority, setCheckInPriority] = useState<
    "normal" | "urgent" | "elderly"
  >("normal");
  ...
  const fetchQueue = useCallback(async () => {
    ...
    try {
      ...
      const ids = [
        ...new Set(allRows.map((r: any) => r.patient_id).filter(Boolean)),
      ];
      const { data: ptRows } = ids.length
        ? await supabase
            .from("patients")
            .select("id, name, gender, age, contact")
            .in("id", ids)
        : { data: [] };
  ...
  const handleSelectPatient = (p: any) => {
    ...
    setPtWeight(p.weight || "");
    setPtBloodGroup(p.blood_group || "");
    setPtAddress(p.address || "");
    setSearchResults([]);
  };
  ...
  const handleCheckIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existing && existing.length > 0) {
        ...
        const { data: neu, error: cErr } = await supabase
          .from("patients")
          .insert([
            {
              name: normalizedName,
  ...
  const updateStatus = async (id: string, status: string) => {
    ...
    try {
      await supabase
        ...
        .update({
          status,
          completed_at: ["done", "skipped"].includes(status)
  ...
  const removeEntry = async (id: string) => {
    setActionId(id);
    try {
      await supabase.from("doctor_queue").delete().eq("id", id);
    } finally {
      setActionId(null);
    }
  };
  ...
  const urgentCount = queue.filter(
    (q) => q.priority === "urgent" && q.status === "waiting",
  ).length;
    ...
    <DashboardLayout>
      <div className={styles.dashboardHeader}>
        <div className={styles.headerTitleGroup}>
          ...
          <p className={styles.headerSubtext}>
            Manage, reorder, check-in, and service patients in the queue.
          </p>
        </div>
        <button
        ...
        {doctors?.map((doc: any) => (
          <button
            ...
            style={{
              padding: "8px 16px",
              borderRadius: "20px",
              border: activeDoctorId === doc.doctor_id ? "none" : "1px solid rgba(23,3,55,0.1)",
              background: activeDoctorId === doc.doctor_id ? "var(--sanctuary-primary)" : "#fff",
              color: activeDoctorId === doc.doctor_id ? "#fff" : "var(--sanctuary-ink-l)",
              fontWeight: 700,
              fontSize: "13px",
              cursor: "pointer",
              whiteSpace: "nowrap"
      ...
      >
        ...
        <button
          ...
          style={{
            marginLeft: "auto",
            background: showDone ? "var(--sanctuary-primary)" : "transparent",
            color: showDone ? "#fff" : "var(--sanctuary-ink-l)",
            border: "1.5px solid rgba(23,3,55,0.1)",
            borderRadius: 30,
            padding: "7px 18px",
            fontSize: 12,
            fontWeight: 700,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 12 }}>
          ...
          <div className={styles.nowServingCard}>
            ...
            <div className={styles.cardContent}>
              ...
              <p className={styles.cardName} style={{ fontSize: 18 }}>
                {serving.patients?.name ?? "Unknown"}
              </p>
              <p
                className={styles.cardMeta}
                style={{ color: "#6d28d9", fontWeight: 600 }}
              >
                {serving.patients?.age}Y · {serving.patients?.gender} ·{" "}
            ...
            <div className={styles.cardActions}>
              ...
              <button
                onClick={() => removeEntry(serving.id)}
                disabled={!!actionId}
                style={{
                  background: "#fee2e2",
                  color: "#dc2626",
        ...
        {waiting.length === 0 && !serving && !loading && (
          ...
          >
            <p style={{ color: "var(--sanctuary-ink-l)", fontSize: 15 }}>
              Queue is empty. Ready for patient check-ins.
            </p>
          </div>
        )}
        ...
        {showDone && doneQueue.length > 0 && (
          ...
          >
            <div
              style={{
                padding: "14px 20px",
                borderBottom: "1px solid rgba(23,3,55,0.05)",
                display: "flex",
                alignItems: "center",
                gap: 10,
              }}
            >
              ...
              <span style={{ fontSize: 12, color: "var(--sanctuary-ink-l)" }}>
                {doneQueue.length} patients
              </span>
            </div>
            {doneQueue.map((p, idx) => (
              <div
                ...
                style={{
                  ...
                  padding: "12px 20px",
                  borderBottom:
                    idx < doneQueue.length - 1
                      ? "1px solid rgba(23,3,55,0.04)"
                      : "none",
                }}
              >
                <div
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    {p.patients?.name ?? p.patient_name ?? "Unknown"}
                  </p>
                  <p
                    style={{
                      fontSize: 11,
      ...
      >
        <Link
          href="/demo1/portal/front-desk/patients"
          style={{
            display: "flex",
            alignItems: "center",
            gap: 10,
            background: "#fff",
            border: "1.5px solid rgba(23,3,55,0.06)",
            borderRadius: 14,
            padding: "16px 20px",
            textDecoration: "none",
            color: "var(--sanctuary-primary)",
        ...
        <Link
          href="/demo1/portal/billing-receipts"
          style={{
            display: "flex",
            alignItems: "center",
            gap: 10,
        ...
        <Link
          href="/demo1/portal/front-desk/clinic-settings"
          style={{
            display: "flex",
            alignItems: "center",
            gap: 10,
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  {(["normal", "elderly", "urgent"] as const).map((p) => (
                    <button
                      key={p}
                      type="button"
                      onClick={() => setCheckInPriority(p)}
                      style={{
                        flex: 1,
                        border: `1.5px solid ${checkInPriority === p ? priorityConfig[p].color : "rgba(23,3,55,0.1)"}`,
                        background:
                          checkInPriority === p ? priorityConfig[p].bg : "#fff",
                        color:
                          checkInPriority === p
