...
function RecordSearch() {
  ...
  const backHref = doctorId
    ? `/demo1/portal/doctor-dashboard?doctorId=${doctorId}&doctorName=${encodeURIComponent(doctorName || "")}`
    : "/demo1/portal/front-desk";
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .or(
          `patient_name.ilike.%${safeQuery}%,patient_phone.ilike.%${safeQuery}%,receipt_number.ilike.%${safeQuery}%`,
        )
                ...
                <div key={record.id} className={styles.resultCard}>
                  ...
                  <div className={styles.resActions}>
                    <button
                      ...
                      onClick={() =>
                        router.push(
                          `/demo1/portal/billing-receipts/view?id=${record.id}${getDoctorParams()}`,
                        )
                      }
                    >
                      View/Print
