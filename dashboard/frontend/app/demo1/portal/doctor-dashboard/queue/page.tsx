...
function useElapsed(startedAt: string | null) {
  ...
  useEffect(() => {
    ...
    let base = new Date(startedAt).getTime();
    if (isNaN(base)) {
      const match = startedAt.match(/^(\d{1,2}):(\d{2})\s*(AM|PM)$/i);
      if (match) {
        ...
        <div className={styles.pageHeader}>
          <div>
            ...
            <p className={styles.pageSubtitle}>
              {displayDoctorName(activeDoctorName)} ·{" "}
          ...
          <div className={styles.queueLayout}>
            ...
            <div className={styles.mainCol}>
              ...
              <section className={styles.tierSection}>
                <div className={styles.tierHeader}>
                  ...
                  {serving && (
                    <ConsultTimer startedAt={serving.check_in_time} />
                  )}
                </div>
                      ...
                      <div className={styles.nsInfo}>
                        ...
                        <p className={styles.nsMeta}>
                          ...
                          <span
                            ...
                            style={{
                              color: priorityConfig[serving.priority].color,
                            }}
                          >
                        ...
                        {serving.notes && (
                          <p className={styles.nsNote}>{serving.notes}</p>
                        )}
                      </div>
                    </div>
                    <div className={styles.nsActions}>
                      <Link
                        href={getDoctorUrl(`/demo1/portal/doctor-dashboard/patients/${serving.patient_id}`)}
                        className={styles.btnOutline}
                      >
                        View Record
                      </Link>
                      <Link
                        href={getDoctorUrl(`/demo1/portal/digital-prescription?patientId=${serving.patient_id}`)}
                      ...
                      <div key={entry.id} className={styles.waitCard}>
                        <div className={styles.waitLeft}>
                          <div
                            ...
                            style={{
                              background: priorityConfig[entry.priority].color,
                            }}
                          >
                            #{entry.token_number}
                          </div>
                          <div className={styles.waitInfo}>
                            ...
                            {entry.notes && (
                              <p className={styles.nsNote}>{entry.notes}</p>
                            )}
                          </div>
                        </div>
                  ...
                  <div className={styles.remainingList}>
                    {remaining.map((entry) => (
                      <div key={entry.id} className={styles.remainingRow}>
                        ...
                        <span
                          className={styles.remainingPriority}
                          style={{
                            color: priorityConfig[entry.priority].color,
                          }}
                        >
              ...
              {queue.length === 0 && !loading && (
                <div className={styles.emptyQueue}>
                  ...
                  <p>
                   ...
                   <Link
                    href={getDoctorUrl("/demo1/portal/front-desk/queue-manager")}
                    className={styles.btnPrimary}
                    style={{ marginTop: 16 }}
                  >
                    Open Queue Manager
            ...
            <aside className={styles.sideCol}>
                  ...
                  {(["urgent", "elderly", "normal"] as const).map((p) => {
                    ...
                    return (
                      <div key={p} className={styles.pBreakRow}>
                        <span style={{ color: priorityConfig[p].color }}>
              ...
              <div className={styles.quickLinksCard}>
                ...
                <Link
                  href={getDoctorUrl("/demo1/portal/digital-prescription")}
                ...
                <Link
                  href={getDoctorUrl("/demo1/portal/doctor-dashboard/patients")}
                  className={styles.qlLink}
                >
                  <svg
                    width="16"
