...
function DischargeSummaryRedesign() {
  ...
  useEffect(() => {
    ...
    const localStep = localStorage.getItem("discharge_summary_draft_step");
    ...
    localStorage.setItem("discharge_summary_draft_step", resolvedStep.toString());
  ...
  useEffect(() => {
    ...
    if (urlStep && !isNaN(parseInt(urlStep))) {
      ...
      setStep(prev => {
        if (prev !== parsed) {
          localStorage.setItem("discharge_summary_draft_step", parsed.toString());
          return parsed;
        }
        return prev;
      });
  ...
  useEffect(() => {
    if (!isInitialized) return;
    localStorage.setItem("discharge_summary_draft_step", step.toString());
  ...
  const saveDraft = useCallback((data: SummaryData) => {
    try {
      ...
      localStorage.setItem("discharge_summary_draft", JSON.stringify(cleanData));
  ...
  const handleDraftStartNew = () => {
    ...
    localStorage.setItem(`discharge_summary_draft_${draftId}`, JSON.stringify(summary));
        ...
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, animation: "fadeIn 0.2s ease-out" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: 32, maxWidth: 400, width: "100%", textAlign: "center", boxShadow: "0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <button type="button" onClick={handleDraftStartNew} style={{ width: "100%", padding: "12px", background: "transparent", color: "#64748b", borderRadius: 12, fontWeight: 600, border: "1px dashed #cbd5e1", cursor: "pointer" }}>Start New Discharge Summary</button>
            </div>
          </div>
        </div>
      )}
      {showFinalizeModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, animation: "fadeIn 0.2s ease-out" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: 32, maxWidth: 400, width: "100%", textAlign: "center", boxShadow: "0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)" }}>
            ...
            <h3 style={{ fontSize: 20, fontWeight: 800, color: "var(--sanctuary-ink)", marginBottom: 8 }}>Discharge Summary Finalized</h3>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {finalizeRecordId && (
                <button type="button" onClick={() => {
                  ...
                  const qs = params.toString();
                  window.open(`/portal/discharge-summary/view?id=${finalizeRecordId}&${qs}`, '_blank');
              ...
              }} style={{ width: "100%", padding: "12px", background: "transparent", color: "#64748b", borderRadius: 12, fontWeight: 600, border: "1px dashed #cbd5e1", cursor: "pointer" }}>Start New Discharge Summary</button>
            </div>
          </div>
        </div>
      )}
