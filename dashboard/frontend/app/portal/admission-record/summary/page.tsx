          ...
          <button
            ...
            onClick={() => {
              const lines = [
                ...
                data.date_admission &&
                  `Admitted: ${formatDate(data.date_admission)}`,
                ...
                data.complaints?.length &&
                  `Complaints: ${data.complaints.join(", ")}`,
                data.findings?.length &&
                  `Findings: ${data.findings.join(", ")}`,
                ``,
                data.vitals_pulse &&
                  `Vitals — Pulse: ${data.vitals_pulse} BPM | SpO₂: ${data.vitals_spo2 || "—"}% | BP: ${data.vitals_bp_sys || "—"}/${data.vitals_bp_dia || "—"} | Temp: ${data.vitals_temp || "—"}°F`,
              ]
                .filter(Boolean)
                .join("\n");
      ...
      <div className={styles.document} id="summary-doc">
        ...
        <div className={styles.docHeader}>
          <div className={styles.docHeaderLeft}>
            ...
            <h1 className={styles.docPatientName}>
          ...
          <div className={styles.docHeaderRight}>
            <div
              className={styles.docSeverityBadge}
              style={{
            ...
            <div className={styles.docMetaSmall}>
              {data.date_admission && (
                <div>Admitted: {formatDate(data.date_admission)}</div>
              )}
        ...
        <div className={styles.infoGrid}>
          <div className={styles.infoCard}>
            ...
            <div className={styles.infoRows}>
              ...
              <div className={styles.infoRow}>
                <span>Doctor</span>
                <strong>{data.doctor ? `Dr. ${data.doctor}` : "—"}</strong>
              </div>
              <div className={styles.infoRow}>
                <span>Admitted</span>
                <strong>{formatDate(data.date_admission)}</strong>
              </div>
            </div>
          </div>
          ...
          <div className={styles.infoCard}>
            ...
            <div className={styles.vitalsGrid}>
              <div
                className={`${styles.vitalBox} ${data.vitals_pulse && parseInt(data.vitals_pulse) > 100 ? styles.vitalAbnormal : ""}`}
              >
              ...
              <div
                className={`${styles.vitalBox} ${data.vitals_spo2 && parseInt(data.vitals_spo2) < 94 ? styles.vitalCritical : ""}`}
              >
                <div className={styles.vitalVal}>
                  {data.vitals_spo2 ? `${data.vitals_spo2}%` : "—"}
                </div>
                <div className={styles.vitalLabel}>SpO₂</div>
              </div>
              <div className={styles.vitalBox}>
                <div className={styles.vitalVal}>
                  {data.vitals_bp_sys && data.vitals_bp_dia
                    ? `${data.vitals_bp_sys}/${data.vitals_bp_dia}`
                    : "—"}
                </div>
              ...
              <div className={styles.vitalBox}>
                <div className={styles.vitalVal}>
                  {data.vitals_temp ? `${data.vitals_temp}°F` : "—"}
                </div>
        ...
        {(data.has_diabetes ||
          ...
          <div className={styles.section}>
            ...
            <div className={styles.comorbiditiesRow}>
              {data.has_diabetes && (
                <span className={styles.comorbidBadge}>Diabetes</span>
              )}
              {data.has_hypertension && (
                <span className={styles.comorbidBadge}>Hypertension</span>
              )}
              {data.has_thyroid && (
                <span className={styles.comorbidBadge}>Thyroid</span>
              )}
              {data.past_surgeries && (
                <span className={styles.comorbidBadgeNeutral}>
            ...
            <div className={styles.investigationTable}>
              ...
              {data.investigations.map((inv: any, i: number) => (
                <div key={i} className={styles.investRow}>
                  ...
                  <span
                    className={`${styles.investStatus} ${inv.status === "Completed" ? styles.statusDone : styles.statusPending}`}
                  >
        ...
        <div className={styles.docFooter}>
          <div>
            MediNest Clinical Record &nbsp;•&nbsp; Generated{" "}
