            ...
            <tr key={i}>
              ...
              <td>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <Link
                    href={`/demo/portal/discharge-summary/view?id=${s.id}${getDoctorParams()}`}
            ...
            <tr key={i}>
              ...
              <td>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  {a.status === "Draft" ? (
                    <Link
                      href={`/demo/portal/admission-record?draftId=${a.id}&patientId=${patientId}${getDoctorParams().replace('?', '&')}`}
                      className={styles.tableAction}
                    >
                      <svg
                        width="14"
                    ...
                    <>
                      <Link
                        href={`/demo/portal/admission-record/view?id=${a.id}${getDoctorParams()}`}
                        className={styles.tableAction}
                      >
                      ...
                      <button
                        onClick={async () => {
                          ...
                          if (data?.id) {
                            window.location.href = `/demo/portal/discharge-summary/view?id=${data.id}${getDoctorParams().replace('?', '&')}`;
                          } else {
                      ...
                      <button
                        onClick={async () => {
                          ...
                          if (data?.id) {
                            window.open(`/demo/portal/discharge-summary/view?id=${data.id}&print=true${getDoctorParams().replace('?', '&')}`, '_blank');
                          } else {
                            alert("Discharge summary not found.");
                          }
                        }}
                    ...
                    <>
                      ...
                      <Link
                        href={`/demo/portal/discharge-summary?patientId=${patientId}&admissionId=${a.id}${getDoctorParams().replace('?', '&')}`}
                        className={styles.tableAction}
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ marginRight: 6 }}>
                          <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
            ...
            <div className={styles.actionGroup}>
              <button
                ...
                onClick={() => {
                  ...
                  const qs = p.toString();
                  router.push(`/demo/portal/digital-prescription${qs ? `?${qs}` : ""}`);
                }}
              >
                <svg
                  width="18"
              ...
              <Link 
                href={`/demo/portal/doctor-dashboard/patients/${patientId}/export${getDoctorParams()}`} 
                target="_blank" 
