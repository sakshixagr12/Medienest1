...
const CLASS_TO_DRUG_MAP = {
  ...
  antacid: {
    meds: ["magnesium hydroxide", "aluminum hydroxide"],
    domain: "gi",
  },
  ...
  antibiotic: {
    meds: ["amoxicillin-clavulanic acid", "azithromycin"],
    domain: "general",
  },
};
...
const DOSAGE_MASTER = {
  ...
  ppi: {
    dose: "40mg",
    freq: "1 tab od (empty stomach)",
    dur: "7 days",
    route: "Oral",
  },
  ors: {
    dose: "1 sachet",
    freq: "Mix in 1L water, sip throughout day",
    dur: "1-2 days",
    route: "Oral",
  },
  antibiotic_u: {
...
const CATEGORY_RED_FLAGS = {
  ...
  gi: [
    "Blood in stool",
    "Persistent vomiting",
    "Severe abdominal tenderness/stiffness",
    "Sudden abdominal pain related to alcohol",
  ],
  neuro: [
    "Sudden facial droop",
    "Slurred speech",
...
const PROTOCOL_MASTER = {
  mi: {
    ...
    avoid: [],
    sev: "emergency",
    tag: "Potential Myocardial Infarction",
  },
  stroke: {
    must: [],
    optional: [],
  ...
  urti: {
    must: ["antipyretic"],
    optional: ["antihistamine", "antitussive"],
    avoid: ["antibiotic", "nsaid"],
    sev: "mild",
    tag: "URTI",
  },
  viral_fever: {
  ...
  sciatica: {
    must: ["nerve_pain"],
    optional: ["analgesic"],
    avoid: ["antihistamine", "antiemetic"],
    sev: "moderate",
    tag: "Sciatica",
  },
  hyperthyroid: {
    must: ["antithyroid"],
    optional: ["beta_blocker"],
    avoid: [],
    sev: "moderate",
    tag: "Hyperthyroid",
  },
...
const INTERACTION_RULES = [
  {
    pair: ["nsaid", "analgesic"],
    action: "block_nsaid",
    reason: "Overlapping analgesia; prioritize Paracetamol safety.",
  },
  {
    pair: ["antihistamine", "bronchodilator"],
    action: "block_antihistamine",
    reason: "Antihistamines can thicken secretions in asthma.",
  },
  {
    pair: ["ppi", "antacid"],
    action: "none",
    reason: "Common clinical combination for rapid relief.",
  },
];
...
const DIAGNOSIS_WARNINGS = {
  ...
  asthma: [
    "Inability to speak in full sentences",
    "Cyanosis (bluish tint to lips/nails)",
    "Silent chest (no breath sounds)",
  ],
  gi: [
    "Severe abdominal rigidity/guarding",
    "Inability to pass gas/stools",
    "Persistent vomiting with dehydration",
  ],
  general: [
    "High fever with stiff neck/confusion",
    "Rapidly spreading purple rash",
...
function rankDiagnosis(list) {
  ...
  return list.sort(
    (a, b) =>
      (common.includes(b.toLowerCase()) ? 1 : 0) -
      (common.includes(a.toLowerCase()) ? 1 : 0),
  );
}
...
function contextGuard(context, classes, cc, findings, diagnosis) {
  ...
  if (
    ...
    return {
      classes: [],
      forcedSeverity: "emergency",
      forcedReason: "Potential TB/Malignancy - Urgent Referral Required.",
    };
  }
  ...
  if (
    ...
    if (
      text.includes("pain") &&
      (text.includes("dizz") || text.includes("faint"))
    ) {
      return {
        classes: [],
        forcedSeverity: "emergency",
        forcedReason: "Potential Ectopic Pregnancy - Immediate ER Referral.",
      };
    }
  ...
  if (
    ...
    return {
      classes: [],
      forcedSeverity: "moderate",
      forcedReason: "Mental Health Counseling/Psychiatry referral prioritized.",
    };
  }
...
function validateBySymptoms(cc, findings, recommendations) {
  ...
  return recommendations.filter((r) => {
    const triggers =
      SYMPTOM_MATCHER[r.drug.toLowerCase()] ||
      SYMPTOM_MATCHER[r.class.toLowerCase()];
...
function safetyLayer(diagnosis, recommendations) {
  ...
  if (diagnosis.includes("diarrhea") || diagnosis.includes("gastroenteritis")) {
    currentMeds = currentMeds.filter(
      (r) => r.class !== "nsaid" && r.class !== "ppi" && r.class !== "laxative",
    );
  }
...
function getWarnings(diagnosis, category) {
  ...
  else if (diag.includes("gerd") || diag.includes("gastritis"))
    warnings.push(...DIAGNOSIS_WARNINGS.gerd);
  else if (diag.includes("asthma") || diag.includes("bronch"))
    warnings.push(...DIAGNOSIS_WARNINGS.asthma);
  else if (
    diag.includes("gastro") ||
    diag.includes("diarrhea") ||
    diag.includes("vomiting")
  )
    warnings.push(...DIAGNOSIS_WARNINGS.gi);
...
function parseClinicalJson(content) {
  try {
    ...
    return {
      severity: "moderate",
      category: "general",
      probable_diagnosis: "Clinical Evaluation Required",
      required_classes: [],
      advice: ["Follow clinical protocols."],
    };
  }
}
...
async function suggestClinicalPath(cc, findings, context = {}) {
  try {
    const { age, gender, specialty, weight, is_pregnant, is_lactating } =
      context;
    ...
    const dxList = [
      clinicalIntent.probable_diagnosis,
      ...(clinicalIntent.differentials || []),
    ];
    const rankedDx = rankDiagnosis(dxList);
    let diagnosis = (rankedDx[0] || "Evaluation Required").toLowerCase();
    let severity = forcedEmergency
      ? "emergency"
      : clinicalIntent.severity || "moderate";
    ...
    const allowed =
      CATEGORY_ALLOWED_DOMAINS[category] || CATEGORY_ALLOWED_DOMAINS["general"];
    ...
    for (const intentKey of classes) {
      ...
      if (meta) {
        const g = meta.meds[0];
        if (
          GLOBAL_AVOID.includes(g.toLowerCase()) ||
          safetyBlocks.includes(g.toLowerCase())
        )
          continue;

        if (!usedGenerics.has(g.toLowerCase())) {
          usedGenerics.add(g.toLowerCase());
          const { data: brands } = await supabase.rpc("search_medicines_v2", {
            search_term: g,
          });
          const safeBrands = (brands || []).filter(
            (b) => !/(inj|iv|im)/i.test(b.name + (b.dosage_form || "")),
          );

          if (safeBrands.length > 0) {
            const doseInfo = DOSAGE_MASTER[intentKey.toLowerCase()] || {
              dose: "As directed",
              freq: "od",
              dur: "3 days",
              route: "Oral",
    ...
    if (severity === "emergency") {
      ...
      diagnosis =
        emergencyReason ||
        diagnosis ||
        "Serious condition — requires urgent evaluation";
    }

    // --- STEP 9: WARNING ENGINE ---
    const finalRecommendations = recommendations.slice(0, 3);
    const adviceList = Array.isArray(clinicalIntent.advice)
      ? clinicalIntent.advice
      : [clinicalIntent.advice || "Follow clinical protocols."];
    const finalAdvice = adviceList.map((a) => `• ${a.trim()}`);
    if (activeTags.length > 0)
      finalAdvice.unshift(`• PROTOCOL: ${activeTags.join(" | ")}`);
    ...
    return {
      ...
      investigations: clinicalIntent.investigations || {
        primary: [],
        secondary: [],
      },
      severity: severity,
...
router.post("/suggest", async (req, res) => {
  ...
  console.log(
    `[AUDIT] CASE: ${age}y ${gender} CC: ${cc} DX: ${suggestions.probable_diagnosis}`,
  );
