...
const isSafeDate = (str) =>
  str && /^\d{4}-\d{2}-\d{2}(T[\d:.Z+-]+)?$/.test(str);
...
const parseMedicines = (medicinesStr) => {
  ...
  try {
    const meds =
      typeof medicinesStr === "string"
        ? JSON.parse(medicinesStr)
        : medicinesStr;
...
router.get("/dashboard", async (req, res) => {
  ...
  try {
    ...
    let currPresQuery = supabase
      .from("prescriptions")
      .select("*, patients(*)")
      .eq("clinic_id", clinic_id);
    let currRecQuery = supabase
      .from("receipts")
      .select("total_amount, printed_at")
      .eq("clinic_id", clinic_id);
    let currQueueQuery = supabase
      .from("doctor_queue")
      .select("serving_started_at, completed_at")
      .eq("clinic_id", clinic_id)
      .eq("status", "done");

    if (hasBase) {
      currPresQuery = currPresQuery
        .gte("created_at", baseStart)
        .lte("created_at", baseEnd);
      currRecQuery = currRecQuery
        .gte("printed_at", baseStart)
        .lte("printed_at", baseEnd);
      currQueueQuery = currQueueQuery
        .gte("completed_at", baseStart)
        .lte("completed_at", baseEnd);
    }
    promises.push(currPresQuery, currRecQuery, currQueueQuery);

    if (doCompare) {
      let prevPresQuery = supabase
        .from("prescriptions")
        .select("*, patients(*)")
        .eq("clinic_id", clinic_id)
        .gte("created_at", compareStart)
        .lte("created_at", compareEnd);
      let prevRecQuery = supabase
        .from("receipts")
        .select("total_amount, printed_at")
        .eq("clinic_id", clinic_id)
        .gte("printed_at", compareStart)
        .lte("printed_at", compareEnd);
      let prevQueueQuery = supabase
        .from("doctor_queue")
        .select("serving_started_at, completed_at")
        .eq("clinic_id", clinic_id)
        .eq("status", "done")
        .gte("completed_at", compareStart)
        .lte("completed_at", compareEnd);
    ...
    const currRevenue = currReceipts.reduce(
      (sum, r) => sum + (r.total_amount || 0),
      0,
    );
    const prevRevenue = prevReceipts.reduce(
      (sum, r) => sum + (r.total_amount || 0),
      0,
    );

    let finalDiagnoses = Object.entries(diagMap)
      .map(([name, count]) => {
        const pCount = prevDiagMap[name] || 0;
        const trend = doCompare ? calcTrend(count, pCount) : null;
        const percentage =
          currData.length > 0 ? Math.round((count / currData.length) * 100) : 0;
        return { diagnosis: name, count, percentage, trend, prevCount: pCount };
      })
      .sort((a, b) => b.count - a.count)
      .slice(0, 20); // allow up to 20

    let finalMedicines = Object.entries(medMap)
      .map(([name, data]) => {
    ...
    const demoAdvanced = Object.entries(ageDiseaseMap)
      .map(([ageGroup, diseases]) => {
    ...
    currReceipts.forEach((r) => {
      if (r.printed_at) {
        ...
        revDateMap[dateStr] =
          (revDateMap[dateStr] || 0) + (r.total_amount || 0);
      }
    });
    const revenueTimeline = Object.keys(revDateMap)
      .sort()
      .map((d) => ({
        date: d,
        amount: revDateMap[d],
    ...
    const calcAvgTime = (queueArr) => {
      const valid = queueArr.filter(
        (q) => q.serving_started_at && q.completed_at,
      );
    ...
    const summaryData = {
      ...
      patientsTrend: doCompare
        ? calcTrend(currData.length, prevData.length)
        : null,
      revenue: currRevenue,
      ...
      consultTrend:
        doCompare && currAvgTime > 0 && prevAvgTime > 0
          ? calcTrend(currAvgTime, prevAvgTime)
          : null,
    };

    const top3Names = finalDiagnoses.slice(0, 3).map((x) => x.diagnosis);
    const diseaseTimeline = Object.keys(dateMap)
      .sort()
      .map((dateStr) => {
    ...
    res.json({
      ...
      data: {
        ...
        demographics: Object.entries(genderMap).map(([k, v]) => ({
          gender: k,
          count: v,
        })),
...
router.get("/patients", requireAuth, requireClinicAccess, async (req, res) => {
  ...
  if (!clinic_id || !diagnosis)
    return res.status(400).json({ success: false, error: "Missing params" });
  if (!isUUID(clinic_id))
    return res
      .status(400)
      .json({ success: false, error: "Invalid clinic_id format" });
  if (baseStart && !isSafeDate(baseStart))
    return res
      .status(400)
      .json({ success: false, error: "Invalid baseStart date" });
  if (baseEnd && !isSafeDate(baseEnd))
    return res
      .status(400)
      .json({ success: false, error: "Invalid baseEnd date" });
  ...
  try {
    ...
    if (hasBase)
      query = query.gte("created_at", baseStart).lte("created_at", baseEnd);
...
router.post("/receipts", requireAuth, requireClinicAccess, async (req, res) => {
  ...
  if (!receiptData || !receiptData.clinic_id) {
    return res
      .status(400)
      .json({ success: false, error: "Missing receipt data or clinic_id" });
  }
