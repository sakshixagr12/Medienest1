...
const CONTRADICTORY_PATTERNS = [
  {
    pair: ["GI_UP", "GI_DOWN"],
    message:
      "Opposite mechanism detected: Promoting bowel movement while simultaneously stopping it.",
  },
  {
    pair: ["GI_SOFT", "GI_DOWN"],
    message:
      "Conflicting intent: Softening stool while attempting to stop diarrhea.",
  },
];
...
function validatePrescription(
  ...
  if (severity.toLowerCase() === "red_flag") {
    flags.push(
      "RED FLAG DETECTED: Suggested referral/urgent care only. No outpatient medications advised.",
    );
  ...
  const isGastro =
    ctx.includes("diarrhea") ||
    ctx.includes("vomiting") ||
    ctx.includes("gastro") ||
    ctx.includes("stomach flu");
  const hasORS = suggestedMeds.some(
    (m) =>
      m.name.toUpperCase().includes("ORS") ||
      m.name.toUpperCase().includes("ELECTROLYTE"),
  );
  ...
  if (isGastro && !hasORS) {
    ...
    flags.push(
      "️ SAFETY RULE: Added mandatory ORS for gastroenteritis management.",
    );
  }
  ...
  const isViral =
    ...
    ctx.includes("cough");
  const isDentalMild =
    (ctx.includes("tooth") || ctx.includes("dental") || ctx.includes("gum")) &&
    !(
      ctx.includes("abscess") ||
      ctx.includes("swelling") ||
      ctx.includes("infection")
    );
  ...
  const filteredMeds = suggestedMeds.filter((m) => {
    ...
    if (isAntibiotic && isViral) {
      flags.push(
        `️ SAFETY BLOCK: Removed ${m.name} (Antibiotics not advised for viral patterns).`,
      );
      return false;
    }
    if (isAntibiotic && isDentalMild) {
      flags.push(
        `️ SAFETY BLOCK: Removed ${m.name} (Antibiotics not advised for non-infectious dental pain).`,
      );
      return false;
    }
    return true;
  ...
  const mustMeds = filteredMeds.filter(
    (m) => (m.tier || "").toUpperCase() === "MUST",
  );
  const optionalMeds = filteredMeds.filter(
    (m) =>
      (m.tier || "").toUpperCase() === "OPTIONAL" ||
      (m.tier || "").toUpperCase() === "SUPPORTIVE",
  );
  ...
  mustMeds.forEach((m) => {
    if (
      validMeds.length < 3 &&
      !validMeds.some((v) => v.name.toLowerCase() === m.name.toLowerCase())
    ) {
      validMeds.push(m);
    }
  });

  const currentFunctionalGroups = new Set(
    validMeds.map((m) => getFunctionalGroup(m.name)),
  );
  ...
  for (const m of optionalMeds) {
    ...
    if (!currentFunctionalGroups.has(fGroup)) {
      ...
      flags.push(
        `PRUNED: Redundant ${m.name} removed to maintain minimal 3-item management.`,
      );
    }
  }
